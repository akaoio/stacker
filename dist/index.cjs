...
class Stacker {
  ...
  findStackerPath() {
    const possiblePaths = [
      // Current package
      path__default.default.join(__dirname$1, "..", "stacker.sh"),
      // Development
      path__default.default.join(process.cwd(), "stacker.sh"),
      // Global installation
      path__default.default.join(process.env.HOME || "", ".local", "bin", "stacker"),
      // System installation
      "/usr/local/bin/stacker"
    ];
    for (const stackerPath of possiblePaths) {
      if (fs__default.default.existsSync(stackerPath)) {
        return stackerPath;
      }
    }
    return "";
  ...
  async service(action) {
    ...
    try {
      return child_process.execSync(script, { encoding: "utf8" });
    } catch (error) {
  ...
  async getConfig(key) {
    ...
    try {
      return child_process.execSync(script, { encoding: "utf8" }).trim();
    } catch (error) {
  ...
  async health(verbose = false) {
    ...
    try {
      const output = child_process.execSync(script, { encoding: "utf8" });
      return {
        healthy: true,
        output: output.trim()
      };
...
exports.StackerUtils = StackerUtils;
